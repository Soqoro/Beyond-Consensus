...
def qualification_key(lock, packages):
    ...
    return digest({'checkpoint': lock['checkpoint'], 'revision': lock['revision'],
        ...
        'settings': {'thinking': True, 'output': 2048, 'total_context': 8192},
...
def require_qualification(lock, packages):
    report = lock.get('decoder_qualification', {})
    if (report.get('status') != 'passed' or report.get('qualification_key') != qualification_key(lock, packages)
